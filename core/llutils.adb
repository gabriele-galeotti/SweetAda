...
package body LLutils is
   ...
   begin
      case Size is
         ...
         when Bits.BIT16 =>
            -- 16-bit swap
            Byte_Swap_16 (Object_Address);
         when Bits.BIT32 =>
            --- 32-bit swap
            Byte_Swap_32 (Object_Address);
         when Bits.BIT64 =>
            -- 64-bit swap
            Byte_Swap_64 (Object_Address);
         when Bits.BITNONE =>
            --- undefined swap
            null;
      end case;
   ...
   begin
      ...
      case Size is
         ...
         when Bits.BIT16 =>
            -- 16-bit swap
            Object_Address := Object_Address + 2;
         when Bits.BIT32 =>
            --- 32-bit swap
            Object_Address := Object_Address + 4;
         when Bits.BIT64 =>
            -- 64-bit swap
            Object_Address := Object_Address + 8;
         when Bits.BITNONE =>
            --- undefined swap
            null;
      end case;
   ...
   begin
      ...
      if MSD then
         Value := (Value and 16#0F#) or (Digit * 2**4);
      else
         Value := (Value and 16#F0#) or Digit;
      end if;
      Success := True;
   end HexDigit_To_U8;
